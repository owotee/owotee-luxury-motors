...
function InventoryPage({
  ...
  const getCarouselCategory = (vehicle) =>
    (vehicle.carousel_category || "auto").toLowerCase();
  ...
  const getVehicleText = (vehicle) => {
    ...
    return `${vehicle.make || ""} ${vehicle.model || ""} ${vehicle.body || ""} ${
      vehicle.badge || ""
    } ${features}`.toLowerCase();
  };

  const autoMatchesCollection = (vehicle, collection) => {
    const text = getVehicleText(vehicle);

    if (collection === "luxury_suvs") {
      return (
        text.includes("suv") ||
    ...
    if (collection === "executive_sedans") {
      return (
        text.includes("sedan") ||
  ...
  const vehicleBelongsToCollection = (vehicle, collection) => {
    const category = getCarouselCategory(vehicle);

    if (category === "none") return false;

    if (category === "auto") {
      return autoMatchesCollection(vehicle, collection);
    }
  ...
  const luxurySuvs = filteredVehicles
    .filter((vehicle) => vehicleBelongsToCollection(vehicle, "luxury_suvs"))
    .slice(0, 8);

  const executiveSedans = filteredVehicles
    .filter((vehicle) =>
      vehicleBelongsToCollection(vehicle, "executive_sedans"),
    )
    .slice(0, 8);

  const sportsAndExotic = filteredVehicles
    .filter((vehicle) => vehicleBelongsToCollection(vehicle, "sports_exotic"))
    .slice(0, 8);
